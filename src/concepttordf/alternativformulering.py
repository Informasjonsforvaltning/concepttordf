"""Module for helper class AlternativFormulering."""
...
class AlternativFormulering(Betydningsbeskrivelse):
    ...
    def _to_graph(self) -> Graph:
        """Transforms the alternative formulation to an RDF graph.

        Returns:
            an RDF graph representing the alternative formulation.
        """
        self._add_alternativformulering_to_graph()
        return self._g

    def _add_alternativformulering_to_graph(self) -> None:
        """Adds the alternative formulation to the Graph _g."""
        self._g.bind("skosno", SKOSNO)

        alt_form_node = BNode()
        self._g.add((alt_form_node, RDF.type, SKOSNO.AlternativFormulering))

        self._add_text_to_graph(alt_form_node)
        self._add_remark_to_graph(alt_form_node)
        self._add_scope_to_graph(alt_form_node)
        self._add_source_to_graph(alt_form_node)
        self._add_example_to_graph(alt_form_node)
    ...
    def to_rdf(self, format: str = "text/turtle") -> str:
        """Convert the alternative formulation to RDF format.
